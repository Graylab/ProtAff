...
N_BOOT="${N_BOOT:-1000}"
OUTPUT_DIR="${OUTPUT_DIR:-analysis/paper_v0}"
...
CMD="python analysis/add_ml_baselines.py --pred ${PRED_CSV}"
CMD="${CMD} --baseline MINT:${MINT_CSV}:interaction_prob:higher"
CMD="${CMD} --baseline CrossAffinity:${CROSSAFFINITY_CSV}:pKd:higher"
CMD="${CMD} --threshold ${THRESHOLD} --n-boot ${N_BOOT} -o ${OUTPUT_DIR}"

[ -f "${GT_CSV}" ] && CMD="${CMD} --gt ${GT_CSV}"
[ -f "${STRUCT_CSV}" ] && CMD="${CMD} --struct ${STRUCT_CSV}"
[ -f "${STRUCT_SEC}" ] && CMD="${CMD} --struct-secondary ${STRUCT_SEC}"
