...
def print_terminal_metrics(final_df, merged_df):
    """Prints AP and Success Rate tables to the terminal."""
    print("\n" + "="*70)
...
def analyze_results(pred_csv, output_dir):
    ...
    for label, (col, lower_is_better) in METRICS_CONFIG.items():
        ...
        aggs.append({'target_id': 'Global (Pooled)', 'AP': get_ap(merged_df['is_binder'], scores_all), 'Method': label})
...
def save_summary(df, output_dir):
    df.to_csv(os.path.join(output_dir, "ap_summary_full.csv"), index=False)
...
def generate_plot(final_df, output_dir):
    ...
    ax = sns.barplot(data=final_df, x='AP', y='target_id', hue='Method', edgecolor='black', palette='muted')
    for container in ax.containers: ax.bar_label(container, fmt='%.2f', padding=3)
    plt.title('Average Precision Comparison', weight='bold'); plt.xlim(0, 1.1); plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'ap_comparison_final.png'), dpi=300); plt.close()
